...
class Entities(TweetTrackerAPIClass):
    ...
    def getEntities_sch(self, queryargs):
        """
        ...
        try:
            ...
            for typ in types:
                if typ not in ["TopUrls", "TopHashtags", "TopMentions"]:
                    return (False,
                            "Types must be a list containing one or more of the following: TopUrls , TopHashtags, TopMentions.")

        except:
            return (
            False, "Types must be a list containing one or more of the following: TopUrls , TopHashtags, TopMentions.")

        try:
            geoboxes = queryargs['geobox'] if 'geobox' in queryargs else []
            boxes = [x.split(",") if x.find(",") != -1 else [x] for x in geoboxes]
            geoboxes = []
            for b in boxes:
                geoboxes += b
        ...
        return True, {"TopUrls": sorted_urls[:min(len(sorted_urls), limit)],
                      "TopHashtags": sorted_hashtags[:min(len(sorted_hashtags), limit)],
                      "TopMentions": sorted_mentions[:min(len(sorted_mentions), limit)]}